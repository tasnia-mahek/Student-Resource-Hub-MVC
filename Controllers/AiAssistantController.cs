        ...
        private async Task<string> BuildReplyAsync(string message)
        {
            ...
            if (lowered.Contains("past paper") || lowered.Contains("past papers") || lowered.Contains("exam paper") || lowered.Contains("question paper"))
            {
                var papers = await _context.PastPapers
                    ...
                    .Take(3)
                    .Select(item => new { item.CourseCode, item.SubjectName, item.Title })
                    .ToListAsync();

                if (papers.Count > 0)
                {
                    var summary = string.Join("; ", papers.Select(item => $"{item.CourseCode} - {item.Title}"));
                    return $"I found recent approved past papers in the system: {summary}. You can open them from the Resources section and filter by course code or subject.";
                }
            ...
            if (lowered.Contains("note") || lowered.Contains("notes") || lowered.Contains("study note"))
            {
                var notes = await _context.Notes
                    ...
                    .OrderByDescending(item => item.CreatedDate)
                    .Take(3)
                    .Select(item => new { item.CourseCode, item.Title, item.SubjectName })
                    .ToListAsync();

                if (notes.Count > 0)
                {
                    var summary = string.Join("; ", notes.Select(item => $"{item.CourseCode} - {item.Title}"));
                    return $"I found these approved study notes: {summary}. They are good starting points for quick revision and topic review.";
                }
            ...
            if (courseCodes.Count > 0)
            {
                ...
                if (parts.Count > 0)
                {
                    return $"For {formattedCodes}, I found relevant resources including {string.Join("; ", parts)}. Use those as your first study sources and then focus on the most recent files.";
                }
            }

            if (lowered.Contains("starter kit") || lowered.Contains("first year") || lowered.Contains("freshman") || lowered.Contains("first-year"))
            {
                return "The First-Year Starter Kit is meant to help you find core foundational subject materials quickly. Start with your department, year, and semester filters, then review the approved notes, lectures, and past papers for each course.";
            }
            ...
            {
                return "A good study plan is: review the lecture, summarize key concepts in your own words, solve recent past papers, and then revisit the notes for weak topics. Focus on active recall and spaced repetition rather than rereading alone.";
            }
